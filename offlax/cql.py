...
class CQL:
    ...
    def __init__(self, rng: jax.random.PRNGKey, actor: Actor, critic: Critic, state_dims: List[int], action_dims: int, gamma: float, tau: float):
        self.actor = actor
        ...
        self.critic1_optimizer_variables = self.critic1_optimizer.init(self.critic1_variables)
        ...
        self.critic2_optimizer_variables = self.critic2_optimizer.init(self.critic2_variables)
    ...
    def get_action(self, state: jnp.ndarray, train: bool=False, rng: jax.random.PRNGKey=None) -> jnp.ndarray:
        state = jax.lax.stop_gradient(state)

        assert rng is not None

        action = self.actor.get_action(self.actor_variables, state, deterministic=not train, key=rng)

        return action

    def get_actor_loss(self, states: jnp.ndarray, actor_variables, critic1_variables, critic2_variables, alpha: float) -> Tuple[jnp.ndarray, jnp.ndarray]:
    ...
    def step(self, experience_batch: List):
        ...
        (actor_loss, log_preds_actor), actor_gradients = jax.value_and_grad(self.get_actor_loss, has_aux=True, argnums=1)(states, self.actor_variables, self.critic1_variables, self.critic2_variables, alpha)
        actor_updates, self.actor_optimizer_variables = self.actor_optimizer.update(actor_gradients, self.actor_optimizer_variables)
        self.actor_variables = optax.apply_updates(self.actor_variables, actor_updates)

        # Update alpha
        alpha_loss = -(self.alpha * jax.lax.stop_gradient(log_preds_actor + self.target_entropy)).mean()
        alpha_loss, alpha_gradients = jax.value_and_grad(self.get_alpha_loss)(self.alpha, log_preds_actor)
        alpha_updates, self.alpha_optimizer_variables = self.alpha_optimizer.update(alpha_gradients, self.alpha_optimizer_variables, self.alpha)
